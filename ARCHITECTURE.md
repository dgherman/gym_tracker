...
- **Config**: `config.py` uses `@lru_cache` for a singleton Settings object from env vars. `BASE_URL` is the single base-URL setting — app redirects **and** the emailed `/invite/confirm` link (`build_confirm_url` falls back to the request host only when `BASE_URL` is unset). The old `APP_BASE_URL` was folded into it.
